    ...
    <>
      <section className="page-shell grid gap-10 pb-16 pt-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-end lg:pb-24 lg:pt-20">
        <div>
          ...
          <h1 className="display mt-7 max-w-4xl text-[clamp(4.1rem,9.4vw,9.4rem)]">Learn the machinery.<br /><span className="text-[var(--amber)]">Keep the boundaries.</span></h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild className="h-12 rounded-none border-[var(--line-strong)] px-5 text-sm" size="lg" variant="outline">
              <Link href="/audit">Audit the conversation</Link>
            </Button>
          </div>
        </div>
        <HeroField />
      ...
      <section aria-label="Course evidence" className="border-y border-[var(--line-strong)] bg-[var(--paper-raised)]">
        <div className="page-shell grid grid-cols-2 divide-x divide-y divide-[var(--line)] md:grid-cols-4 md:divide-y-0">
          {[["22", "connected modules"], ["9", "audited turns"], ["52", "traceable sources"], ["6", "recomputable labs"]].map(([number, label]) => (
      ...
      <section className="page-shell py-20 lg:py-28">
        <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
          <div>
            ...
            <p className="mt-7 max-w-md leading-7 text-[var(--ink-muted)]">The curriculum is ordered by dependencies, not release hype. Each lesson moves from your intuition to its formal object, a counterexample, a calculation, and retrieval practice.</p>
          </div>
      ...
      <section className="bg-[var(--blue)] py-20 text-[#f4f0e7] lg:py-28">
        <div className="page-shell">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="eyebrow !text-[#b8d5e6]">A conversation, repaired</p>
              <h2 className="display mt-5 text-[clamp(3rem,6vw,6.2rem)]">Good questions.<br />Overconfident answers.</h2>
            </div>
            <div>
              <p className="max-w-2xl text-lg leading-8 text-[#d9e7ee]">The supplied dialogue found important questions about self-supervision, reinforcement learning, search, world models, and verification. The assistant repeatedly converted them into false certainty and praise. The audit keeps the questions and replaces the claims.</p>
              <div className="mt-10 grid gap-5">
                {[
                  ...
                  <div className="grid gap-3 border-t border-[#6a96b2] pt-5 sm:grid-cols-2" key={wrong}>
                    <span className="flex gap-2 text-sm text-[#b8d5e6]"><X aria-hidden="true" className="mt-0.5 size-4 shrink-0" />{wrong}</span>
              ...
              <Link className="mt-9 inline-flex items-center gap-2 border-b border-[#f4f0e7] pb-1 text-sm no-underline" href="/audit">Read all nine turns <ArrowRight aria-hidden="true" className="size-4" /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="page-shell py-20 lg:py-28">
        <div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          ...
          <p className="max-w-md text-sm leading-6 text-[var(--ink-muted)]">Estimated reading time: 13 hours, before exercises. Progress is stored only on this device.</p>
        </div>
        <ModuleMap compact />
      </section>
    </>
